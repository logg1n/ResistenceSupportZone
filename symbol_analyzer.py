...
class SymbolAnalyzer:
    ...
    def __init__(self, symbol: str):
        ...
        # Активные зоны по ТФ
        self.active_zones: Dict[str, List[Zone]] = {
    ...
    async def process_candle(self, timeframe: str, candle_data: Dict):
        ...
        try:
            if timeframe not in self.market_states:
                app_logger.warning(f"Unsupported timeframe {timeframe} for {self.symbol}")
                return
            ...
            if (datetime.now() - self.last_analysis_time[timeframe]).total_seconds() >= interval:
            ...
            for zone in zones:
                ...
                for confirm_tf in config.CONFIRMATION_TIMEFRAMES:
                    if confirm_tf in self.market_states and confirm_tf != timeframe:
                        ...
                        if ConfirmationSystem.confirm_across_timeframes(zone, confirm_state):
    ...
    def get_performance_stats(self) -> Dict:
        ...
        return {
            "symbol": self.symbol,
            **self.performance_stats,
            "active_zones_total": sum(len(zones) for zones in self.active_zones.values())
        }
