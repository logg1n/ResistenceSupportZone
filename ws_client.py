...
class BybitWebSocketClient:
    ...
    async def _subscribe_to_channels(self) -> bool:
        ...
        if len(subscriptions) > 10:
            logger.warning(f"Слишком много каналов ({len(subscriptions)}), берем первые 10")
        ...
        try:
            ...
            if "op" in data:
                ...
                if op == "subscribe":
                    if data.get("success", False):
                        logger.info(f"✅ Подтверждение подписки: {data.get('args', [])}")
                    else:
            ...
            if topic:
                ...
                if self.stats["messages_received"] <= 10:
                    data_type = "свечи" if "kline" in topic else "сделки" if "trade" in topic else "стакан"
    ...
    async def _connection_handler(self):
        ...
        while self.running and reconnect_attempt < self.config.MAX_RECONNECT_ATTEMPTS:
            try:
                logger.info(f"Подключение к WebSocket... (попытка {reconnect_attempt + 1})")
    ...
    async def stop(self):
        ...
        # Итоговая статистика
        elapsed = (datetime.now() - self.stats["start_time"]).total_seconds()
        logger.info(f"📊 ИТОГО: {self.stats['messages_received']} сообщений за {elapsed:.1f} сек")
...
def parse_arguments():
    ...
    parser.add_argument(
        "--redis-host",
        default="localhost",
        help="Хост Redis (по умолчанию: localhost)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=6379,
        help="Порт Redis (по умолчанию: 6379)"
    )
